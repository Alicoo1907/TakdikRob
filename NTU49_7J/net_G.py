...
# Parent dizindeki transformer_utils'u bulabilmesi icin
...
class ActFormer_Generator(nn.Module):
    """
    ActFormer-based Generator for motion generation (NTU Optimized)
    """
    ...
    def forward(self, z, y):
        ...
        if len(z.shape) == 2:
            ...
            # NTU FIX: Z=64, T=64 durumunda karisiklik olmamasi icin 
            # eger zaten (B, T, Z) ise dokunma.
            if z.shape[1] == self.T and z.shape[2] == self.Z:
                pass # Correct shape
            elif z.shape[1] == self.Z and z.shape[2] == self.T:
                z = z.transpose(1, 2) # Only swap if definitely flipped
        else:
        ...
        y_emb = self.class_embedding(y).unsqueeze(1)  # (B, 1, embed_dim)
        x = torch.cat((y_emb, x), 1)  # (B, T+1, embed_dim)
